#!/usr/bin/env bash
# Idempotent Cloud Agent bootstrap for fintech-fun (Bazel 8.2 + Go 1.24).
# Installs the toolchain, warms the Bazel cache, and pre-pulls Neo4j/Qdrant
# images so per-boot start.sh is fast. Must terminate and be safe to re-run.
set -euo pipefail
...
log "Warm Bazel: build everything"
bazel build //...
