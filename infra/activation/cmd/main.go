...
func main() {
	if err := cmd(impl.HTTP{}, os.Stdout).Execute(); err != nil {
		os.Exit(2)
	}
}

func cmd(checker contract.Checker, stdout io.Writer) *cobra.Command {
	cmd := &cobra.Command{
		Use:           "activation",
	...
	cmd.AddCommand(
		...
		new_step_cmd(
			"smoke",
			"GET /healthz /fold /search /v1/graph",
			nil,
			cobra.ExactArgs(1),
			func(api string) error {
		...
		new_step_cmd(
			"gold",
			"HTTP fold rows match Notion gold tables",
			nil,
			cobra.ExactArgs(1),
			func(api string) error {
